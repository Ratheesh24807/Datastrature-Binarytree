...
public class BinaryTree3<K extends Comparable <K>> {
	...
	private boolean searchKey(MyBinaryNode<K> left, K key) {
		if(left == null) return false;
		if(left.key == key) return true;
		if(searchKey(left.left, key)) return true;
		return searchKey(left.right, key);
	}
